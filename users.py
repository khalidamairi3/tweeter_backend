...
def post_user():
    ...
    if email and email!="" and username and username!="" and  password and  password != "" and bio and bio!="" and birthdate and birthdate !="": 
        try:
            conn = mariadb.connect(user=dbcreds.user,password=dbcreds.password, host=dbcreds.host,port=dbcreds.port, database=dbcreds.database)
            cursor = conn.cursor()
...
def patch_user():
    ...
    try:
        ...
        if email !="" and email !=None and loginToken !="" and loginToken !=None :
            cursor.execute("UPDATE users SET email= ?  WHERE id = ( SELECT user_id from `session`  WHERE login_token = ? )",[email,loginToken,])
        if username !="" and username !=None and loginToken !="" and loginToken !=None :
            cursor.execute("UPDATE users SET username= ?  WHERE id = ( SELECT user_id from `session`  WHERE login_token = ? )",[username,loginToken,])
        if password !="" and password !=None and loginToken !="" and loginToken !=None :
            cursor.execute("UPDATE users SET password= ?  WHERE id = ( SELECT user_id from `session`  WHERE login_token = ? )",[password,loginToken,])
        if bio !="" and bio !=None and loginToken !="" and loginToken !=None :
            cursor.execute("UPDATE users SET bio= ?  WHERE id = ( SELECT user_id from `session`  WHERE login_token = ? )",[bio,loginToken,])
        if birthdate !="" and birthdate !=None and loginToken !="" and loginToken !=None :
            cursor.execute("UPDATE users SET birthday= ?  WHERE id = ( SELECT user_id from `session`  WHERE login_token = ? )",[birthdate,loginToken,])
        conn.commit()
        result= cursor.rowcount
