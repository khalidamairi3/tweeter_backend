...
def get():
    ...
    if tweetId:
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT c.id, c.tweet_id, u.id , u.username , c.content , c.created_at FROM users u  INNER JOIN comment c ON u.id = c.user_id WHERE c.tweet_id=?" ,[tweetId,])
